		...
		mapping_table.each do |k, v|
			if (not object.blank?) || (v[:value][0] == 64)  || (v[:type] == :HAS_CHILD_MULTI_AND_SELF) # @ == 64
				case v[:type]
                    ...
                    when :OBJECT_FIELD_ORIG:
                        returns_hash[k] = object[v[:value]].blank? ? nil: object[v[:value]]
					...
					when :DB_FIELD_MODIFIED_ARRAY_ID:
						...
						if  object[v[:value]].blank?
							...
							prd_attribute_ids = object[v[:value]].split(",")
							array_ = []
							prd_attribute_ids.each do |prd_attribute_id|

                                prd_attribute = PrdAttribute.find_by_id(prd_attribute_id)
								hash_ = Hash.new
								hash_['product_id'] = prd_attribute.products[0].id
								hash_['prd_type'] = prd_attribute.prd_type 
								hash_['name'] = prd_attribute.name
                    ...
                    when :HAS_CHILD_MULTI_PRD_ATTRIBUTE:

                        array_1 = []

                        if not object.relation_subs.blank?

                            object.relation_subs.each do |relation_sub|
                                hash_1 = Hash.new
                                hash_1[
                                    'id' => relation_sub.id,
                                    'parent_id' => relation_sub.parent_id,
                                    'prd_attribute_id' => relation_sub.prd_attribute_id
                                ]
                            end
                        end
					...
					when :HAS_CHILD_MULTI_PRODUCT:
						...
						if not object.products.blank?
                            ...
                            #hash_1 = Hash.new
                            #hash_1['product_id'] = object.product
                            #array_1 << hash_1
