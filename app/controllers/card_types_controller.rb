class CardTypesController < ApplicationController
  # GET /card_types
  # GET /card_types.xml
  def index
    @card_types = CardType.all

    respond_to do |format|
      format.html # index.html.erb
      format.xml  { render :xml => @card_types }
      format.json  { render :json => @card_types }
    end
  end

  # GET /card_types/1
  # GET /card_types/1.xml
  def show

    # @card_type = CardType.find(params[:id])
    @card_type = CardType.find(:all, :conditions => {:price_model => params[:id]})

    respond_to do |format|
      format.html # show.html.erb
      format.xml  { render :xml => @card_type }
      format.json  { render :json => @card_type }
    end
  end

  # GET /card_types/new
  # GET /card_types/new.xml
  def new
    @card_type = CardType.new

    respond_to do |format|
      format.html # new.html.erb
      format.xml  { render :xml => @card_type }
      format.json  { render :json => @card_type }
    end
  end

  # GET /card_types/1/edit
  def edit
    @card_type = CardType.find(params[:id])
  end

  # POST /card_types
  # POST /card_types.xml
  def create
    @card_type = CardType.new(params[:card_type])

    what_custom = WhatCustom.new
    what_custom.subfctr_name = 'RECHARGE_'+params[:card_type][:price_model]
    what_custom.description = params[:card_type][:price_model_nm]
    what_custom.save!

    what_custom_factor = WhatCustomFactor.new
    what_custom_factor.name = 'RECHARGE_CODE'
    what_custom_factor.value = params[:card_type][:price_model]
    what_custom_factor.operation = '='
    what_custom_factor.what_custom_id = what_custom.id
    what_custom_factor.save!

    what_factor = WhatFactor.new
    what_factor.fctr_code = 'PC_RECHARGE'
    what_factor.name = 'VOUCHER_'+params[:card_type][:price_model]
    what_factor.description = 'RECHARGE by Card Type'
    what_factor.code_factor_id = 106
    what_factor.rating_group= 0
    what_factor.what_custom_id = what_custom.id
    what_factor.save!

    respond_to do |format|
      if @card_type.save
        format.html { redirect_to(@card_type, :notice => 'CardType was successfully created.') }
        format.xml  { render :xml => @card_type, :status => :created, :location => @card_type }
        format.json  { render :json => @card_type, :status => :created, :location => @card_type }
      else
        format.html { render :action => "new" }
        format.xml  { render :xml => @card_type.errors, :status => :unprocessable_entity }
        format.json  { render :json => @card_type.errors, :status => :unprocessable_entity }
      end
    end
  end

  # PUT /card_types/1
  # PUT /card_types/1.xml
  def update
    #@card_type = CardType.find(params[:id])
    @card_type = CardType.find(:first, :conditions => {:price_model => params[:id]})

    respond_to do |format|
      if @card_type.update_attributes(params[:card_type])
        format.html { redirect_to(@card_type, :notice => 'CardType was successfully updated.') }
        format.xml  { head :ok }
        format.json  { head :ok }
      else
        format.html { render :action => "edit" }
        format.xml  { render :xml => @card_type.errors, :status => :unprocessable_entity }
        format.json  { render :xml => @card_type.errors, :status => :unprocessable_entity }
      end
    end
  end

  # DELETE /card_types/1
  # DELETE /card_types/1.xml
  def destroy
    #@card_type = CardType.find(params[:id])
    @card_type = CardType.find(:first, :conditions => {:price_model => params[:id]})
    @card_type.destroy

    respond_to do |format|
      format.html { redirect_to(card_types_url) }
      format.xml  { head :ok }
      format.json  { head :ok }
    end
  end
end
